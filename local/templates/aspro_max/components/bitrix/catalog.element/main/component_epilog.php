...
<?if($arResult["ID"]):?>
	...
	$bShowInfographicsTab = (($arParams['SHOW_INFOGRAPHICS'] == 'Y') ? ++$i : '');
	...
	<?foreach($arBlockOrder as $code):?>
		...
		<?//tabs?>
		<?elseif($code == 'tabs'):?>
			<?if($bShowDetailTextTab || $bShowPropsTab || $bShowInfographicsTab || $bShowVideoTab || $bShowHowBuyTab || $bShowPaymentTab || $bShowDeliveryTab || $bShowStoresTab || $bShowCustomTab || $bShowReviewsTab || $bShowBuyServicesTab):?>
				<div class="ordered-block js-store-scroll tabs-block" data-hash="Y">
							...
							<ul class="nav nav-tabs font_upper_md">
								...
								<?foreach($arTabOrder as $value):?>
									...
									<?if($value == "stores"):?>
										...
										<?endif;?>
									<?endif;?>
									<?//show infographics block?>
									<?if($value == "infographics"):?>
										<?if($bShowInfographicsTab):?>
											<li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>">
												<a href="#infographics" data-toggle="tab"><?=$arParams["TAB_INFOGRAPHICS_NAME"];?></a>
											</li>
										<?endif;?>
									<?endif;?>
									...
									<?if($value == "custom_tab"):?>
										<?if($bShowCustomTab):?>
											<li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>">
												<a href="#custom_tab" data-toggle="tab"><?=$arParams["TAB_DOPS_NAME"];?></a>
											</li>
										<?endif;?>
									<?endif;?> 
									<?//show reviews block?>
					...
					<div class="tab-content<?=($i <= 1 ? ' not_tabs' : '')?>">
						...
						<?foreach($arTabOrder as $value):?>
							...
							<?if($value == "infographics"):?>
								<?if($bShowInfographicsTab):?>
									<div class="tab-pane <?=$value;?> <?=(!($iTab++) ? 'active' : '')?>" id="infographics">
										<?if($i == 1):?>
											<div class="ordered-block__title option-font-bold font_lg">
												<?=$arParams["TAB_INFOGRAPHICS_NAME"];?>
											</div>
										<?endif;?>
										<?$APPLICATION->ShowViewContent('PRODUCT_INFOGRAPHICS_INFO')?>
									</div>
								<?endif;?>
							<?endif;?>
							<?//show video block?>
		...
		<?//infographics?>
		<?elseif($code == 'infographics' && $bShowInfographicsTab):?>
			<div class="ordered-block <?=$code?>">
				<div class="ordered-block__title option-font-bold font_lg">
					<?=$arParams["TAB_INFOGRAPHICS_NAME"];?>
				</div>
...
<? // clickon set
if ($APPLICATION->GetCurDir()=='/catalog-test/'){
if(!empty($arResult['ID'])){
    ?>
    <? if(\Bitrix\Main\Loader::includeModule('clickon.set')){ ?>
        <?php
        $idElement = $arResult['ID']; // для которого ищем комплекты
        ?>
        <div>
            <? $APPLICATION->IncludeComponent(
                "clickon.set:set.element",
                "",
                array(
                    "ELEMENT_ID" => "14144",
                    "BASKET_URL" => $arParams["BASKET_URL"],
                    "OPEN_NEW_TAB" => "Y",
                    "USE_SET_SELECT" => "N",
                    "SET_SELECT_TITLE" => "У этого товара есть несколько комплектов:(ИЗ ПАРАМЕТРОВ)",
                    "SET_SELECT_VIEW" => "select",
                    ...
                    "PAGER_SHOW_ALL" => "N",
                    "PAGER_BASE_LINK_ENABLE" => "N",
                    "THEME_RESOLVER" => "aspro_max",
                    'THEME' => [
                        'BASE_COLOR' => '#0158bf', // основной цвет (рамки, правой панели)
                        'TEXT_ON_BASE_COLOR' => '#000000', // текст цвета на фоне основгого цвета
                        'DISCOUNT_VALUE_COLOR' => '#2c80e3', // цвет блока "Выгода"
                        'DISCOUNT_VALUE_TEXT_COLOR' => '#FFFFFF', // цвет текста на фоне блока "Выгода"
                        //'SECONDARY_COLOR' => '#FFFFFF',
                        //'BORDER_COLOR' => '#000000',
                        'OFFER_SELECTOR_BG_COLOR' => '#f4f4f4',
                    ],
                ),
                false
            ); ?>
            ...
            <style>
                .cs-basket .cs__basket__price__position__text,
                .cs-basket .cs__basket__price__profit .cs__price-text,
                .cs-basket .cs__basket__price__profit .cs__price-value,
                .cs-basket .cs__basket__price__old .cs__price-text,
                .cs-basket .cs__basket__price__old .cs__price-value,
                .cs-basket .cs__basket__price__new .cs__price-text,
                .cs-basket .cs__basket__price__new .cs__price-value,
                .cs__content .cs__current-item__profit span,
                .cs__content .cs__item__info__discount__profit span,
                .cs-basket .cs__basket__price__profit .cs__basket__price__profit__text,
                .cs-basket .cs__basket__price__profit .cs__basket__price__profit__value,
                .cs-basket .cs-basket-position,
                .cs-basket .cs-basket-price-old .price-text,
                .cs-basket .cs-basket-price-old .price-value,
                .cs-basket .cs-basket-price-diff-total .price-text,
                .cs-basket .cs-basket-price-diff-total .price-value,
                .cs-basket .cs-basket-price-new .price-text,
                .cs-basket .cs-basket-price-new .price-value,
                .cs-basket .cs-basket-add-btn,
                .cs-tabs .cs-tabs-item.active .cs-tab-item
                {
