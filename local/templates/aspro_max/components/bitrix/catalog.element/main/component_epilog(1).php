...
<?if($arResult["ID"]):?>
	...
	$bShowInfographicsTab = (!empty($templateData['INFOGRAPHICS']) || !empty($templateData['INFOGRAPHICS_IFRAME']) ? ++$i : '');
								...
								<?foreach($arTabOrder as $value):?>
									...
									<?if($value == "infographics"):?>
										<?if($bShowInfographicsTab):?>
											<li class="bordered rounded3 <?=(!($iTab++) ? 'active' : '')?>">
												<a href="#infographics" data-toggle="tab">
													<?=$arParams["TAB_INFOGRAPHICS_NAME"];?>
													<?if(count($templateData["INFOGRAPHICS"]) > 1):?>
														<span class="count empty">&nbsp;(<?=count($templateData["INFOGRAPHICS"])?>)</span>
													<?endif;?>
												</a>
											</li>
										<?endif;?>
