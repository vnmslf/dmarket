...
<?$templateData['SHOW_ADDITIONAL_TAB'] = true;?>
...
<?if (!empty($arResult["PROPERTIES"]["CML2_PICTURESINFO"]["VALUE"])): ?>
<?if($arResult && $arResult['PROPERTIES']['CML2_PICTURESINFO']['VALUE']):?>
	<?$countPhoto = count($arResult['PROPERTIES']['CML2_PICTURESINFO']['VALUE']);?>
		...
		<div class="bordered rounded3">
			<div class="gallery-big">
				<div class="big-gallery-block">
					<div class="owl-carousel owl-theme owl-bg-nav short-nav owl-carousel--button-offset-half" data-plugin-options='{"items": "1", "autoplay" : false, "autoplayTimeout" : "3000", "smartSpeed":1000, "dots": true, "nav": true, "loop": false, "index": true, "margin": 0, "dotsContainer": false, "dots": true, "relatedTo": ".gallery-thumbs .owl-carousel"}'>
						<?foreach($arResult['PROPERTIES']['CML2_PICTURESINFO']['VALUE'] as $photoId):?>
							<?
			...
			<?if ($countPhoto > 1):?>
				<div class="gallery-thumbs hidden-xs">
					...
					?>
					<div class="thmb text-center owl-carousel owl-carousel--light owl-carousel--button-wide owl-carousel--button-offset-half" data-plugin-options='{"items": "7", "nav": true, "loop": false, "clickTo": ".gallery-big .owl-carousel", "dots": false, "margin": 30, "mouseDrag": true, "pullDrag": true, "rewind": true, "responsive": {"768": {"items": 4<?=($countPhoto > 4 ? $mouseDragTrue : $mouseDragFalsy)?>}, "992": {"items": 5<?=($countPhoto > 5 ? $mouseDragTrue : $mouseDragFalsy)?>}, "1100": {"items": 6<?=($countPhoto > 6 ? $mouseDragTrue : $mouseDragFalsy)?>}, "1300": {"items": 7<?=($countPhoto > 7 ? $mouseDragTrue : $mouseDragFalsy)?>}}}'>
						<?foreach($arResult['PROPERTIES']['CML2_PICTURESINFO']['VALUE'] as $photoId):?>
							<?
							$arImage = CFile::GetFileArray($photoId);
...
<?//infographics tab?>
<?if($arParams['SHOW_INFOGRAPHICS'] == 'Y'):?>
<?$templateData['SHOW_INFOGRAPHICS'] = true;?>
	<?$this->SetViewTarget('PRODUCT_INFOGRAPHICS_INFO');?>
<?/*$APPLICATION->IncludeFile(SITE_DIR."include/additional_products_description.php", array(), array("MODE" => "html", "NAME" => GetMessage('CT_BCE_CATALOG_ADDITIONAL_DESCRIPTION')));*/?>
<?if (!empty($arResult["PROPERTIES"]["CML2_PICTURESINFO"]["VALUE"])): ?>
<?if($arResult && $arResult['PROPERTIES']['CML2_PICTURESINFO']['VALUE']):?>
	<?$countPhoto = count($arResult['PROPERTIES']['CML2_PICTURESINFO']['VALUE']);?>
