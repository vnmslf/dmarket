...
if(!isset($arProperty["NUM_AMOUNT"])){
	...
	if($arParams["SHOW_GENERAL_STORE_INFORMATION"] != "Y"){
		foreach($arResult["STORES"] as $pid => $arProperty){
			...
			if($arResult["STORES"][$pid]["PHONE"]){
				$arResult["STORES"][$pid]["PHONE"] = implode(', ', array_merge(explode(",", $arResult["STORES"][$pid]["PHONE"]), (array)Solution::unserialize($arStore['UF_PHONES']) ?: array()));
			}elseif(isset($arStore["UF_PHONES"])){
				$arResult["STORES"][$pid]["PHONE"] = implode(', ', (array)Solution::unserialize($arStore['UF_PHONES']));
			}
