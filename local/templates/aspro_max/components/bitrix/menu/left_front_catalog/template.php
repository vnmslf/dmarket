...
<?if($arResult):?>
	...
	<div class="menu_top_block catalog_block">
		...
		<?endif;?>
		<?$classMenuPosition = $arTheme["MENU_POSITION"]["VALUE"] == 'COMBINED' ? 'm_line m_combined' : 'm_' . strtolower($arTheme["MENU_POSITION"]["VALUE"]);?>
		<ul class="menu dropdown">
			<?foreach($arResult as $key => $arItem){?>
				...
				?>
				<li class="full <?=($arItem["CHILD"] ? "has-child" : "");?> <?=($arItem["SELECTED"] ? "current" : "");?><?($arChildItem["CHILD"] ? "opened" : "")?> <?=$classMenuPosition?> v_<?=strtolower(CMax::GetFrontParametrValue("MENU_TYPE_VIEW"));?>">
